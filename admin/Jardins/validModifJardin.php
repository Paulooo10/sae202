...
$id = $_POST['id_admin']; //ID DU NOUVEL AUTEUR CHOISI PAR L'ADMIN
...
if (!isset($_POST['id_admin']) || !is_numeric($id)) {
    ...
    header("Location: modifier.php&id=$id_jardin");
    die();
}
verifAuteur($id, "modifier.php"); //script qui verifie si l'auteur existe dans la bdd
if (!isset($_POST['id_admin']) || !is_numeric($id_utilisateur_de_base)) {
    $_SESSION['erreur'] = "Erreur dans l'ID de l'auteur original";
    header("Location: modifier.php&id=$id_jardin");
    die();
}
verifAuteur($id_utilisateur_de_base, "modifier.php"); //script qui verifie si l'auteur existe dans la bdd
...
$nouvelleImage = ajoutImageSecondaire("photo1", "modifier.php&id=$id_jardin");
$nouvelleImage2 = ajoutImageSecondaire("photo2", "modifier.php&id=$id_jardin");
$nouvelleImage3 = ajoutImageSecondaire("photo3", "modifier.php&id=$id_jardin");
...
if ($_SESSION['erreur'] == "") {
    ...
    $id_utilisateur_de_base = $db->quote($id_utilisateur_de_base);
    $id = (int)$id;
